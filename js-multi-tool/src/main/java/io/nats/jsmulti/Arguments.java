...
class Arguments{
    ...
    public final int latencyThreads;
    public final boolean connShared;
    public final boolean latencyConnShared;
    public final long jitter;
    public final long latencyJitter;
    ...
    public String toString() {
        ...
        String lThreads = latencyAction == null ? "" : "\n  latency threads (-ld)     " + latencyThreads;
        String lJitter = latencyAction == null ? "" : "\n  latency jitter (-lj)      " + latencyJitter;
        ...
        if (latencyThreads > 1) {
            s += "\n  latency connection (-ln): " + (latencyConnShared ? SHARED : INDIVIDUAL);
        }
    ...
    public Arguments(String[] args) {
        ...
        int _threads = 1;
        int _latencyThreads = 1;
        boolean _connShared = true;
        boolean _latencyConnShared = true;
        long _latencyJitter = 0;
        long _jitter = 0;
            ...
            for (int x = 0; x < args.length; x++) {
                ...
                switch (arg) {
                    ...
                    case "-d":
                        ...
                        break;
                    case "-ld":
                        _latencyThreads = asNumber(args, ++x, 10, "number of latency threads");
                        break;
                    case "-j":
                        _jitter = asNumber(args, ++x, 10_000, "jitter");
                        break;
                    case "-lj":
                        _latencyJitter = asNumber(args, ++x, 10_000, "jitter");
                        break;
                    case "-n":
                        _connShared = trueIfNot(args, ++x, "individual");
                        break;
                    case "-ln":
                        _latencyConnShared = trueIfNot(args, ++x, "individual");
                        break;
                    case "-kp":
        ...
        if (_latencyAction != null) {
            if (!contains(PUB_ACTIONS, _action)) {
                error("Valid publish action required while running latency!");
            }
            if (!contains(SUB_ACTIONS, _latencyAction)) {
                error("Valid subscribe latency action required!");
            }
        }

        if (_threads == 1 && contains(QUEUE_ACTIONS, _action)) {
            error("Queue subscribing requires multiple threads!");
        }

        if (_latencyThreads == 1 && contains(QUEUE_ACTIONS, _latencyAction)) {
            error("Queue subscribing requires multiple threads!");
        }
        ...
        threads = _threads;
        latencyThreads = _latencyThreads;
        connShared = _connShared;
        latencyConnShared = _latencyConnShared;
        jitter = _jitter;
        latencyJitter = _latencyJitter;
